...
if (!firstName || !grade) {
  ...
  if (existingCount >= MAX_CHILDREN) {
    return res.status(403).render("parent/child_limit", {
      user: req.user,
 ...
 for (const rule of rules) {
  ...
  // 🎓 If parent already PAID, also assign paid quizzes for this child's grade
  ...
  if (parentUser && parentUser.subscriptionStatus === "paid") {
    ...
    // Mark child as consumer enabled
