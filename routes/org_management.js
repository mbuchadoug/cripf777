...
router.get('/org/:slug/dashboard', ensureAuth, async (req, res) => {
  try {
   ...
   // const Question = require('../models/question');
    ...
    const getIcon = (s = '') => {
      if (s.includes('governance')) return '🏛️';
      if (s.includes('finance')) return '💰';
      if (s.includes('law')) return '⚖️';
      if (s.includes('education')) return '🎓';
      if (s.includes('technology')) return '💻';
    ...
    // ─────────────────────────────────────────────
    // STEP 2 — BUILD SERIES STRUCTURE
    // ─────────────────────────────────────────────
    const quizzesBySeries = {};

    for (const q of questions) {
      const category = q.category || 'general';
      const pillar = q.pillar || 'responsibility';
      const series = q.series || 'general-series';

      if (!quizzesBySeries[series]) {
        quizzesBySeries[series] = {
          category,
          pillar,
          series,
          quizzes: []
      ...
      quizzesBySeries[series].quizzes.push({
    ...
    // ─────────────────────────────────────────────
    // STEP 3 — BUILD CATEGORY META (PRIMARY)
    // ─────────────────────────────────────────────
    let categoryMap = {};
    ...
    for (const s of Object.values(quizzesBySeries)) {
      ...
      if (!cat || cat === 'out-of-scope') continue;
      ...
      categoryMap[cat].count += s.quizzes.length;
    }

    let allCategoryMeta = Object.values(categoryMap)
      .filter(c => c.count > 0)
      .sort((a, b) => b.count - a.count);

    let allCategorySlugs = allCategoryMeta.map(c => c.slug);

    // ─────────────────────────────────────────────
    // HARD FALLBACK — GUARANTEED CATEGORY BUILD
    // ─────────────────────────────────────────────
    if (!allCategoryMeta.length) {
      const fallbackMap = {};
      ...
      for (const q of questions) {
        ...
        if (!fallbackMap[cat]) {
          fallbackMap[cat] = {
            ...
            label: slugToLabel(cat),
            icon: getIcon(cat),
            pillar: q.pillar || 'responsibility',
            count: 0
          };
        }
        fallbackMap[cat].count += 1;
      }

      allCategoryMeta = Object.values(fallbackMap);
      allCategorySlugs = allCategoryMeta.map(c => c.slug);
    }

    // ─────────────────────────────────────────────
    // STEP 4 — BUILD PILLAR STRUCTURE
    // ─────────────────────────────────────────────
    const quizzesByPillarArray = [];

    for (const pillar of PILLAR_ORDER) {
      const categories = allCategoryMeta.filter(c => c.pillar === pillar);

      const totalQuizzes = categories.reduce((sum, c) => sum + c.count, 0);

      if (totalQuizzes > 0) {
        quizzesByPillarArray.push({
          pillarSlug: pillar,
          pillarLabel: slugToLabel(pillar),
          pillarIcon: '🧠',
          totalQuizzes,
    ...
    res.render('org/dashboard', {
      org: { slug },
      isCripfcntSchool: true,

      quizzesBySeries,
      quizzesByPillarArray,

      categoryMeta: allCategoryMeta,
