...
// routes/dashboard.js (or wherever your dashboard route is)

router.get("/org/:slug/dashboard", ensureAuth, async (req, res) => {
  try {
    const { slug } = req.params;
    const selectedCategory = (req.query.category || "").toLowerCase();

    const org = await Organization.findOne({ slug });
    if (!org) return res.status(404).send("Organization not found");

    // ─────────────────────────────────────────────
    // 1. LOAD ALL QUIZZES (COMPREHENSION TYPE)
    // ─────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────
    // 2. BUILD CATEGORY COUNTS
    // ─────────────────────────────────────────────
    const categoryMap = {};

    for (const q of questions) {
      const category = (q.category || "").toLowerCase().trim();
    ...
    // ─────────────────────────────────────────────
    // 3. FILTER ZERO + SORT DESC
    // ─────────────────────────────────────────────
    let categoryMeta = Object.values(categoryMap)
      .filter(c => c.count > 0)
      .sort((a, b) => b.count - a.count);

    // ─────────────────────────────────────────────
    // 4. FILTER QUESTIONS BY CATEGORY (IF SELECTED)
    // ─────────────────────────────────────────────
    let filteredQuestions = questions;

    if (selectedCategory) {
      filteredQuestions = questions.filter(q => {
        return (q.category || "").toLowerCase() === selectedCategory;
      });
    }

    // ─────────────────────────────────────────────
    // 5. GROUP QUESTIONS INTO QUIZZES (SERIES SAFE)
    // ─────────────────────────────────────────────
    const quizMap = {};

    for (const q of filteredQuestions) {
      const key = q.series || q._id.toString();

      if (!quizMap[key]) {
        quizMap[key] = {
          title: q.seriesTitle || q.title || "Untitled Quiz",
          questions: [],
          category: (q.category || "").toLowerCase()
        };
      }

      quizMap[key].questions.push(q);
    }
