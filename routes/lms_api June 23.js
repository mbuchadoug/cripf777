...
/**
 * Generate a certificate PDF using Puppeteer.
 * Falls back to pdfkit if Puppeteer is unavailable.
 * Returns { filepath, filename, method }
 *
 * FIXES:
 *  - Removed --single-process and --no-zygote (crash on Linux kernel 6+)
 *  - Added headless:"new" (Puppeteer ≥ 20)
 *  - waitUntil "networkidle2" (avoids font CDN timeout)
 *  - Uses new PMP-inspired buildCertificateHtml from certificateTemplate.js
 *  - emulateMediaType("print") for correct @page sizing
 */
async function generateCertificatePdf({ name, orgName, moduleName, quizTitle, score, percentage, date, req }) {
  ...
  try {
    ...
    if (puppeteer) {
      // ✅ CORRECT flags for Linux VPS (kernel 5.18+)
      // ❌ REMOVED: --single-process  → SIGTRAP crash on modern kernels
      // ❌ REMOVED: --no-zygote       → only needed with --single-process
      const launchOpts = {
        headless: "new",
        args: [
          "--no-sandbox",
      ...
      try {
        ...
        // networkidle2: waits until ≤2 network connections for 500ms (font CDN safe)
        await page.setContent(html, { waitUntil: "networkidle2", timeout: 30000 });
        // Extra second for fonts to render after network settles
        await new Promise(r => setTimeout(r, 1000));
        // "print" media type so @page rules and print-color-adjust apply
    ...
    console.warn("[generateCertificatePdf] Puppeteer failed, falling back to pdfkit:", err?.message || err);
  }
  ...
  try {
    ...
    return await new Promise((resolve, reject) => {
      try {
        ...
        doc.moveDown(1.2);
        doc.fontSize(24).fillColor("#111").text("Certificate of Achievement", { align: "center" });
        doc.moveDown(1.2);
        doc.fontSize(12).fillColor("#444").text("This is to certify that", { align: "center" });
        doc.moveDown(0.8);
        doc.fontSize(20).fillColor("#000").text(name || "Recipient", { align: "center" });
        doc.moveDown(0.8);
        doc.fontSize(12).fillColor("#444").text("has successfully completed", { align: "center" });
        doc.moveDown(0.6);
        doc.fontSize(14).fillColor("#000").text(quizTitle || moduleName || "", { align: "center" });
        doc.moveDown(1.2);
        if (score != null)      { doc.fontSize(11).fillColor("#333").text(`Score: ${score}`, { align: "center" }); doc.moveDown(0.4); }
        if (percentage != null) { doc.fontSize(11).fillColor("#333").text(`Grade: ${percentage}%`, { align: "center" }); doc.moveDown(1.2); }
    ...
    console.warn("[generateCertificatePdf] pdfkit fallback failed:", err?.message || err);
    return null;
  }
}
