...
// ─────────────────────────────────────────────────────────────────────────────
// ADDITIONS TO routes/supplierAdmin.js
// ─────────────────────────────────────────────────────────────────────────────
//
// ADD these imports at the top of supplierAdmin.js (with the other imports):
//
//   import CategoryPreset from "../models/categoryPreset.js";
//   import { SUPPLIER_CATEGORIES } from "../services/supplierPlans.js";
//   import { TEMPLATES, getPresetCategories } from "../services/supplierProductTemplates.js";
//
// Then ADD all the routes below BEFORE your existing module.exports or router export.
// ─────────────────────────────────────────────────────────────────────────────

// ── GET /zq-admin/presets ─────────────────────────────────────────────────────
// List all categories with preset status (admin dashboard page)
router.get("/presets", isSupplierAdmin, async (req, res) => {
  try {
    // Get all DB presets
    const dbPresets = await CategoryPreset.find().lean();
    const dbMap = Object.fromEntries(dbPresets.map(p => [p.catId, p]));

    // Get static fallback presets
    const staticPresets = getPresetCategories();
    const staticMap = Object.fromEntries(staticPresets.map(p => [p.id, p]));

    // Merge: all known categories + their preset status
    ...
    res.render("admin/presets", { cats: allCats, user: req.session.adminUser });
  } catch (err) {
    console.error("[Admin Presets List]", err);
    res.status(500).send("Error loading presets");
  }
});

// ── GET /zq-admin/presets/:catId ──────────────────────────────────────────────
// Edit form for a specific category preset
router.get("/presets/:catId", isSupplierAdmin, async (req, res) => {
  try {
    const { catId } = req.params;
    const catDef = SUPPLIER_CATEGORIES.find(c => c.id === catId);
    if (!catDef) return res.status(404).send("Category not found");
      ...
      if (staticTemplate) {
        ...
        preset = {
          ...
          subcatMap: staticTemplate.subcatMap
            ? Object.entries(staticTemplate.subcatMap).map(([label, products]) => ({ label, products }))
            : [],
          isActive: true,
    ...
    res.render("admin/preset_edit", {
      cat: catDef,
      preset,
      source,
      subcatDefs: catDef.subcats || [],
      user: req.session.adminUser
    });
  } catch (err) {
    console.error("[Admin Preset Edit GET]", err);
    res.status(500).send("Error loading preset");
  }
});

// ── POST /zq-admin/presets/:catId ─────────────────────────────────────────────
// Save a preset (create or update)
router.post("/presets/:catId", isSupplierAdmin, async (req, res) => {
  try {
    const { catId } = req.params;
    const catDef = SUPPLIER_CATEGORIES.find(c => c.id === catId);
    if (!catDef) return res.status(404).json({ error: "Category not found" });

    // Parse products (one per line or comma-separated from textarea)
    const rawProducts = (req.body.products || "")
      .split(/[\n,]+/)
      .map(p => p.trim())
      .filter(Boolean);

    // Parse prices from JSON (the admin UI sends JSON array)
    let prices = [];
    try {
    ...
    // Parse subcatMap from JSON
    let subcatMap = [];
    try {
      const raw = JSON.parse(req.body.subcatMap || "[]");
      subcatMap = raw.filter(s => s.label && Array.isArray(s.products));
    } catch (_) {
      subcatMap = [];
    }
    ...
    // Also update in-memory static templates so it takes effect immediately
    // without server restart
    const { setTemplateForCategory } = await import("../services/supplierProductTemplates.js");
...
// Remove a preset (category goes back to "no preset")
router.delete("/presets/:catId", isSupplierAdmin, async (req, res) => {
  try {
...
// Toggle isActive without editing full preset
router.patch("/presets/:catId/toggle", isSupplierAdmin, async (req, res) => {
  try {
...
// ── GET /zq-admin/presets/:catId/json ─────────────────────────────────────────
// Return preset as raw JSON (for copy/export)
router.get("/presets/:catId/json", isSupplierAdmin, async (req, res) => {
  try {
    const preset = await CategoryPreset.findOne({ catId: req.params.catId }).lean();
    if (!preset) {
      const staticTemplate = TEMPLATES[req.params.catId];
      if (!staticTemplate) return res.status(404).json({ error: "No preset found" });
