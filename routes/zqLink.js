...
//
// Schools:   GET  /s/:slug        → Smart Card page
//            POST /s/:slug/capture → Lead capture + redirect to WA deep-link
//            GET  /s/:slug/qr      → Print-ready QR poster
//
// Suppliers: GET  /p/:slug        → Supplier Smart Card page
//            POST /p/:slug/capture → Lead capture + redirect to WA deep-link
//
// Mount in app.js:
...
//
// SchoolProfile must have: zqSlug, zqLinkViews, zqLinkConversions
...
function _facilitySnippet(facs = []) {
  const M = { swimming_pool:"Pool", science_lab:"Science Lab", computer_lab:"Computer Lab",
    library:"Library", sports_fields:"Sports Fields", wifi:"Wi-Fi", boarding_house:"Boarding",
    transport:"Transport", gymnasium:"Gym", auditorium:"Hall", medical_centre:"Medical", chapel:"Chapel" };
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /s/:slug  — School Smart Card page
// ─────────────────────────────────────────────────────────────────────────────
router.get("/:slug", async (req, res) => {
  try {
    ...
    const canonical  = `${BASE_URL}/s/${slug}`;
    const pageTitle  = `${school.schoolName} | ZimQuote`;
    const desc       = _schoolDesc(school);
    const ogImage    = school.ogImageUrl || `${BASE_URL}/og/school-default.png`;
...
<div class="card">
  ...
  <div class="cs">
    ...
    <div class="ab">
      <button class="btn pri" onclick="go('fees','${slug}','${source}')"><span class="bi">💵</span><span><strong>Request Fees</strong><small>Get the full fee schedule</small></span></button>
      <button class="btn sec" onclick="go('visit','${slug}','${source}')"><span class="bi">📅</span><span><strong>Book a School Visit</strong><small>Schedule a tour</small></span></button>
      <button class="btn sec" onclick="showGS()"><span class="bi">📝</span><span><strong>Ask About a Place</strong><small>Check space availability</small></span></button>
      ${hasDocs?`<button class="btn sec" onclick="go('pdf','${slug}','${source}')"><span class="bi">📄</span><span><strong>Download School Profile</strong><small>PDF sent to your WhatsApp</small></span></button>`:""}
      <button class="btn sec" onclick="go('enquiry','${slug}','${source}')"><span class="bi">💬</span><span><strong>Send Enquiry</strong><small>Any other question</small></span></button>
    </div>
    <div id="gs" style="display:none;margin-top:12px">
      ...
      <select id="gi" style="width:100%;padding:11px 12px;border:1px solid #c8dac8;border-radius:9px;font-size:14px;color:#1a2e1a;background:#fff;margin-bottom:10px"><option value="">Select grade...</option>${gradeOpts}</select>
      <button class="btn pri" style="margin-top:0" onclick="goGrade('${slug}','${source}')"><span class="bi">📝</span><span><strong>Continue on WhatsApp</strong></span></button>
    </div>
  </div>
  ...
  <div class="sr">
    ...
    <a href="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(zqUrl)}" class="sb fb" target="_blank" rel="noopener">Facebook</a>
    <a href="https://twitter.com/intent/tweet?text=${sMsg}" class="sb tw" target="_blank" rel="noopener">Twitter / X</a>
    <button class="sb cp" id="cpb" onclick="cpLink('${esc(zqUrl)}')">📋 Copy</button>
  </div>
  <div class="pw"><a href="${BASE_URL}" target="_blank" rel="noopener">Powered by <strong>ZimQuote</strong> · Zimbabwe's school finder</a></div>
</div>
<script>
function showGS(){var e=document.getElementById("gs");e.style.display="block";e.scrollIntoView({behavior:"smooth",block:"nearest"});}
function goGrade(slug,src){var g=document.getElementById("gi").value;if(!g){alert("Please select a grade.");return;}go("place",slug,src,g);}
async function go(action,slug,src,grade){
  ...
  try{
    var r=await fetch("/s/"+slug+"/capture",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
    var d=await r.json();if(d.redirect)window.location.href=d.redirect;
  }catch(e){window.location.href="https://wa.me/${BOT_NUMBER}?text="+encodeURIComponent("ZQ:SCHOOL:${String(school._id)}:"+action);}
}
function cpLink(u){navigator.clipboard.writeText(u).then(function(){var b=document.getElementById("cpb");b.textContent="✅ Copied!";setTimeout(function(){b.textContent="📋 Copy";},2000);});}
</script>`}));
  } catch (err) {
...
// ─────────────────────────────────────────────────────────────────────────────
// POST /s/:slug/capture  — Lead capture endpoint
// ─────────────────────────────────────────────────────────────────────────────
router.post("/:slug/capture", async (req, res) => {
  try {
    ...
    // Save the structured lead record
    ...
    // Notify school admin (fire-and-forget, never blocks response)
    ...
    // Build deep-link payload the bot will intercept
...
router.get("/:slug/qr", async (req, res) => {
  try {
    ...
    const canonical = `${BASE_URL}/${isSupplier?"p":"s"}/${slug}`;
    ...
    const feeStr = entity.fees?.term1 ? `$${entity.fees.term1}/term`
...
async function _serveSupplierPage(req, res, slug) {
  try {
    ...
    const stars      = (supplier.reviewCount||0)>0 ? `⭐ ${Number(supplier.rating).toFixed(1)} (${supplier.reviewCount} reviews)` : "⭐ New listing";
    const prices     = isService
      ? (Array.isArray(supplier.rates)&&supplier.rates.length ? supplier.rates.slice(0,3).map(r=>`${r.service}: ${r.rate}`).join(" · ") : "Rates on request")
      : (Array.isArray(supplier.prices)&&supplier.prices.length ? supplier.prices.slice(0,3).map(p=>`${p.product} $${Number(p.amount).toFixed(2)}`).join(" · ") : "Prices on request");
    const sMsg = encodeURIComponent(`🏪 ${name} – ${supplier.area?supplier.area+", ":""}${supplier.city}\n${products}\n\nChat & order:\n👉 ${canonical}\n\n_Found via ZimQuote_`);

    res.setHeader("Cache-Control","public, max-age=60");
    res.send(_shell({ pageTitle:`${name} | ZimQuote`, desc:`${name} in ${supplier.city}. ${products}. Chat now on ZimQuote.`,
      ogImage:`${BASE_URL}/og/supplier-default.png`, canonical, body:`
<div class="card">
  <div class="ch">
  ...
  <div class="cs">
    ...
    <p class="al">What would you like to do?</p>
    <div class="ab">
      <button class="btn pri" onclick="goS('enquiry','${slug}','${source}')"><span class="bi">💬</span><span><strong>Send Enquiry</strong><small>Ask about products or services</small></span></button>
      <button class="btn sec" onclick="goS('fees','${slug}','${source}')"><span class="bi">💰</span><span><strong>Request Price List</strong><small>Get full pricing</small></span></button>
    </div>
  </div>
  <div class="sr">
    <span class="sl">Share:</span>
  ...
  <div class="pw"><a href="${BASE_URL}" target="_blank" rel="noopener">Powered by <strong>ZimQuote</strong></a></div>
</div>
<script>
async function goS(action,slug,src){
  ...
  var body={parentName:name,action:action,source:src};
  document.querySelector(".cs").style.opacity="0.6";
  try{var r=await fetch("/p/"+slug+"/capture",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});var d=await r.json();if(d.redirect)window.location.href=d.redirect;}
  catch(e){window.location.href="https://wa.me/${BOT_NUMBER}?text="+encodeURIComponent("ZQ:SUPPLIER:${String(supplier._id)}:"+action);}
}
function cpLink(u){navigator.clipboard.writeText(u).then(function(){var b=document.getElementById("cpb");b.textContent="✅ Copied!";setTimeout(function(){b.textContent="📋 Copy";},2000);});}
</script>`}));
  } catch (err) {
