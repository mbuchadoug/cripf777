...
import {
  fetchReportData, calcTotals, buildWhatsAppSummary, fmt
...
export async function runMonthlyReportMetaEnhanced({ biz, from }) {
  ...
  const baseSummary = buildWhatsAppSummary({
    biz, label: "Monthly Report", periodLabel: monthName, data, totals, branchName
  });

  const weekLines = weeks.map((w, i) =>
    `  Wk${i + 1} ${w.label.padEnd(8)}  In:${fmt(w.in, cur).padStart(10)}  ${w.profit >= 0 ? "✅" : "❌"}${fmt(w.profit, cur).padStart(8)}`
  ).join("\n");
...
VS ${prevMonthName.toUpperCase()}
  ...
  await sendText(from, baseSummary + extraText);

  // ── Generate professional HTML report ──────────────────────────────────────
  try {
    const { generateReportPDF } = await import("./reportPDF.js");
    const { filename } = await generateReportPDF({
      biz, reportType: "Monthly Report", periodLabel: monthName,
      branchName, data, totals, prevTotals, weeks
    });
    const site = (process.env.SITE_URL || "").replace(/\/$/, "");
    await sendDocument(from, { link: `${site}/docs/generated/reports/${filename}`, filename });
  } catch (e) { console.error("[MONTHLY REPORT HTML]", e.message); }
