...
// ── Smart Link menu (My Store → My Smart Link) ────────────────────────────────
// Routes: My Store → "📲 My Smart Link" → shows stats, benefits, share captions, QR
if (a === "sup_get_my_link") {
  const { findSupplierByPhone } = await import("./supplierSearch.js");
  const phone    = from.replace(/\D+/g, "");
  const supplier = await findSupplierByPhone(phone);
  if (supplier) {
    // sc_smart_link_<supplierId> is handled by handleSellerChatAction (sc_ prefix)
    return handleSellerChatAction({
      from,
      action: `sc_smart_link_${String(supplier._id)}`,
      biz,
      saveBiz: saveBizSafe.bind(null, biz)
    });
  }
