...
export async function sendBroadcastTemplate({
  ...
  if (!phones.length) return { sent: 0, failed: 0, skipped: 0, results: [] };
  ...
  for (const rawPhone of phones) {
    const phone = _normalizeZimPhone(rawPhone);
    ...
    if (dryRun) {
      results.push({ phone, status: "dry_run" });
      sent++;
      continue;
    }
    ...
    try {
      ...
      console.log(`[BROADCAST] ✅ ${templateName} → ${phone}`);
