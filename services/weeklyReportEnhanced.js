...
`;
  ...
  const baseMsg = buildReportMessage({
    biz, label: "Weekly Report",
    periodLabel, data, totals, branchName
  });
  ...
  try {
    const reportNum = `RPT-W-${Date.now()}`;
    const { filename } = await generatePDF({
      type: "receipt", number: reportNum, date: start,
      billingTo: `Weekly Report — ${periodLabel}${branchName ? ` (${branchName})` : ""}`,
  ...
  } catch (e) { console.error("[WEEKLY REPORT PDF]", e.message); }
