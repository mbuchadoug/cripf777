...
export async function notifyAdminPhotoReview({
  ...
  try {
    ...
    console.warn(`[ADMIN PHOTO REVIEW] template failed: ${err.message}. Falling back to sendText.`);
    try {
      await sendText(
        adminPhone,
      ...
      console.warn(`[ADMIN PHOTO REVIEW] fallback sendText also failed: ${e.message}`);
    }
  }
}
