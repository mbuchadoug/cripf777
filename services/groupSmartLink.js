...
// ══════════════════════════════════════════════════════════════════════════════
// ─── TUTOR GROUP SMART LINKS ──────────────────────────────────────────────────
//
// A "tutor group" bundles multiple PRIVATE TUTORS (SupplierProfile with
// profileType: "tutor") under one shareable link.
// Deep link format: https://wa.me/<BOT>?text=ZQ:TGROUP:<slug>
//
// Chatbot flow (mirrors the school group, but tutor-styled):
//   1. Visitor sends "ZQ:TGROUP:<slug>"
//   2. handleTutorGroupSmartLink() shows a WhatsApp list of tutors
//      with action IDs  zqtg_tut_<tutorId>
//   3. Visitor taps a tutor → handleTutorGroupTap() → showTutorProfile()
//      (identical experience to opening a tutor from a parent search:
//       pitch → FLYERS → BROCHURES → profile card → ✉️ Send Enquiry)
//   4. Bottom CTA row "🧑‍🏫 List as a Tutor" → handleTutorGroupTap()
//      → startSupplierRegistration() (the picker prominently offers Private Tutor)
//
// Admin is notified (non-blocking) when:
//   • the group link is OPENED           → _notifyAdminTutorGroupOpened()
//   • a tutor profile is TAPPED          → _notifyAdminTutorTapped()
//   • someone taps "List as a Tutor"     → _notifyAdminTutorGroupRegTap()
//
// Admin CRUD (called from supplierAdmin.js):
//   createTutorGroup / getAllTutorGroups / getTutorGroupBySlug /
//   addTutorToGroup / removeTutorFromGroup / deleteTutorGroup /
//   setTutorGroupTagline / validateGroupSlug (shared)
//
// Link builders:
//   buildTutorGroupDeepLink / buildTutorGroupQrImageUrl
// ══════════════════════════════════════════════════════════════════════════════

// ─── Mongoose model ────────────────────────────────────────────────────────────
const tutorGroupSchema = new mongoose.Schema(
  {
    slug:      { type: String, required: true, unique: true, lowercase: true, trim: true },
    name:      { type: String, required: true, trim: true },
    tagline:   { type: String, default: "" },
    ...
    // tutors: ordered array of { tutorId, order }
    // tutorId references a SupplierProfile whose profileType === "tutor"
    tutors: [
      {
        tutorId: { type: mongoose.Schema.Types.ObjectId, ref: "SupplierProfile" },
        order:   { type: Number, default: 0 }
      }
    ]
  },
...
const TutorGroup = mongoose.models.TutorGroup
  || mongoose.model("TutorGroup", tutorGroupSchema);

// ─── Link builders ─────────────────────────────────────────────────────────────

export function buildTutorGroupDeepLink(slug) {
  const payload = `ZQ:TGROUP:${slug}`;
  return `${BOT_WA_URL}?text=${encodeURIComponent(payload)}`;
}

export function buildTutorGroupQrImageUrl(slug, sizePx = 400) {
  const link    = buildTutorGroupDeepLink(slug);
  const encoded = encodeURIComponent(link);
  return `https://chart.googleapis.com/chart?cht=qr&chs=${sizePx}x${sizePx}&chl=${encoded}&choe=UTF-8`;
}
...
export async function getAllTutorGroups() {
  return TutorGroup.find({}).sort({ createdAt: -1 }).lean();
}

export async function getTutorGroupBySlug(slug) {
  return TutorGroup.findOne({ slug: String(slug).toLowerCase().trim() }).lean();
}

export async function createTutorGroup({ slug, name, tagline = "" }) {
  const s = String(slug).toLowerCase().trim();
  const err = validateGroupSlug(s);
  if (err) throw new Error(err);
  const existing = await TutorGroup.findOne({ slug: s });
  if (existing) throw new Error(`Slug "${s}" is already taken.`);
  return TutorGroup.create({ slug: s, name: String(name).trim(), tagline: String(tagline).trim() });
}

export async function deleteTutorGroup(slug) {
  return TutorGroup.findOneAndDelete({ slug: String(slug).toLowerCase().trim() });
}

export async function setTutorGroupTagline(slug, tagline) {
  return TutorGroup.findOneAndUpdate(
    { slug: String(slug).toLowerCase().trim() },
    { $set: { tagline: String(tagline || "").trim() } },
    { new: true }
  );
...
/**
 * Add a tutor to a tutor group by phone number.
 * Only SupplierProfiles with profileType "tutor" may be added.
 */
export async function addTutorToGroup(slug, phone) {
  const SupplierProfile = (await import("../models/supplierProfile.js")).default;
  const cleanPhone = String(phone || "").replace(/\D/g, "");
  const tutor = await SupplierProfile.findOne({ phone: cleanPhone }).lean();
  if (!tutor) throw new Error(`No listing found with phone ${cleanPhone}`);
  if (tutor.profileType !== "tutor")
    throw new Error(`${tutor.businessName || cleanPhone} is not a private tutor. Only tutors can be added to a tutor group.`);

  const group = await TutorGroup.findOne({ slug: String(slug).toLowerCase().trim() });
  if (!group) throw new Error(`Tutor group "${slug}" not found`);

  const alreadyIn = (group.tutors || []).some(
    t => String(t.tutorId) === String(tutor._id)
  );
  if (alreadyIn) throw new Error(`${tutor.businessName} is already in this group.`);

  const nextOrder = Math.max(0, ...(group.tutors || []).map(t => t.order ?? 0)) + 1;
  group.tutors.push({ tutorId: tutor._id, order: nextOrder });
  await group.save();
  return group;
}

/**
 * Remove a tutor from a tutor group by phone number.
 */
export async function removeTutorFromGroup(slug, phone) {
  const SupplierProfile = (await import("../models/supplierProfile.js")).default;
  const cleanPhone = String(phone || "").replace(/\D/g, "");
  const tutor = await SupplierProfile.findOne({ phone: cleanPhone }).lean();
  if (!tutor) throw new Error(`No listing found with phone ${cleanPhone}`);
...
// ─── Chatbot: handle ZQ:TGROUP:<slug> ─────────────────────────────────────────

/**
 * Called when a visitor sends "ZQ:TGROUP:<slug>".
...
export async function handleTutorGroupSmartLink({ from, slug, biz, saveBiz, offset = 0 }) {
  try {
    ...
    // Track view (non-blocking)
    TutorGroup.findByIdAndUpdate(group._id, { $inc: { viewCount: 1 } }).catch(() => {});

    // Notify admin that the group link was opened - non-blocking, never breaks flow
    _notifyAdminTutorGroupOpened(group, from).catch(() => {});

    const SupplierProfile = (await import("../models/supplierProfile.js")).default;

    // Is the visitor already a registered tutor? (hide the register CTA for them)
    const cleanPhone = String(from || "").replace(/\D/g, "");
    const visitorIsTutor = !!(await SupplierProfile.findOne(
      { phone: cleanPhone, profileType: "tutor" }, { _id: 1 }
    ).lean());

    // ── Build CTA row (only for non-tutor visitors) ───────────────────────────
    const ctaLabel = (group.ctaText || "🧑‍🏫 List as a Tutor").slice(0, 24);
    const ctaRow = visitorIsTutor ? null : {
      id:          `zqtg_register_${group.slug}`,
      title:       ctaLabel,
      description: "Tap to list yourself and get found by parents".slice(0, 72)
    };
    ...
    if (!tutorIds.length) {
      ...
      const rows = ctaRow ? [ctaRow] : [];
      if (!rows.length) {
        await sendText(from, `❌ *${group.name}* has no tutors yet. Check back soon!`);
        return true;
      }
      await sendList(from, header, rows);
      return true;
    ...
    const tutors = await SupplierProfile.find(
      { _id: { $in: tutorIds }, profileType: "tutor", active: true },
      { businessName: 1, location: 1, subjects: 1, hourlyRate: 1, _id: 1 }
    ).lean();
    ...
    if (!orderedTutors.length) {
      const header = `😕 No active tutors in *${group.name}* right now.`;
      const rows = ctaRow ? [ctaRow] : [];
      if (!rows.length) {
        await sendText(from, `😕 No active tutors in *${group.name}* right now. Try again later.`);
        return true;
      }
      await sendList(from, header, rows);
      return true;
    ...
    // Subject label helper (best-effort; falls back to raw id)
    ...
    const tutorRows = pageTutors.map(t => {
      ...
      const desc = [subj, rate].filter(Boolean).join(" · ")
        || [t.location?.area, t.location?.city].filter(Boolean).join(", ");
      return {
        id:          `zqtg_tut_${t._id}`,
        title:       (t.businessName || "Tutor").slice(0, 24),
    ...
    console.error("[TUTOR GROUP SMART LINK ERROR]", err.message);
    return false;
  }
}

// ─── Chatbot: handle zqtg_tut_* and zqtg_register_* ───────────────────────────
...
export async function handleTutorGroupTap({ from, action, biz, saveBiz }) {
  ...
  if (/^zqtg_register_/i.test(actionStr)) {
    const slug = actionStr.replace(/^zqtg_register_/i, "").trim();
    return handleTutorGroupRegFlow({ from, slug, biz, saveBiz });
  }

  // ── Route: tutor profile tap ──────────────────────────────────────────────
  if (/^zqtg_tut_/i.test(actionStr)) {
    try {
      const tutorId = actionStr.replace(/^zqtg_tut_/i, "").trim();
      if (!tutorId || tutorId.length !== 24) {
        console.warn(`[TUTOR GROUP TAP] Invalid tutorId from action="${action}"`);
        return false;
      }
      console.log(`[TUTOR GROUP TAP] from=${from} tutorId=${tutorId}`);

      // Notify admin that a tutor profile was tapped from a group link (non-blocking).
      // View counting + tutor-side notification are handled inside showTutorProfile,
      // so we do NOT double-increment here.
      const SupplierProfile = (await import("../models/supplierProfile.js")).default;
      SupplierProfile.findById(tutorId).lean().then((t) => {
        if (t) _notifyAdminTutorTapped(t, from).catch(() => {});
      }).catch(() => {});

      // Open the school-style tutor profile - this loads the pitch, any FLYERS
      // (images) and any BROCHURES (documents), then the profile card + Enquiry.
      const { showTutorProfile } = await import("./tutorSearch.js");
      await showTutorProfile(from, tutorId, biz, saveBiz, { source: "tutor_group" });
      return true;

    } catch (err) {
      console.error("[TUTOR GROUP TAP ERROR]", err.message, err.stack);
      try { await sendText(from, "❌ Could not load tutor profile. Please try again."); } catch (_) {}
      return true; // prevent chatbotEngine fall-through
    }
  }
...
// ─── Chatbot: tutor self-registration from group CTA ──────────────────────────

/**
 * Called when a visitor taps "🧑‍🏫 List as a Tutor".
 * Delegates to startSupplierRegistration() - the same proven flow used by the
 * school and supplier group CTAs. The listing-type picker prominently offers
 * "🧑‍🏫 Private Tutor", so the visitor can register as a tutor in one tap.
 */
export async function handleTutorGroupRegFlow({ from, slug, biz, saveBiz }) {
  try {
    console.log(`[TUTOR GROUP REGISTER] from=${from} slug=${slug}`);

    const group = await TutorGroup.findOne({ slug: String(slug).toLowerCase().trim() }).lean();
    if (group) {
      TutorGroup.findByIdAndUpdate(group._id, { $inc: { registrationTaps: 1 } }).catch(() => {});
      _notifyAdminTutorGroupRegTap({ group, visitorPhone: from }).catch(() => {});
    }
    ...
    console.error("[TUTOR GROUP REGISTER ERROR]", err.message, err.stack);
    try { await sendText(from, "❌ Something went wrong. Please type *menu* to continue."); } catch (_) {}
    return true;
  }
}

// ─── Tutor group admin notification helpers ───────────────────────────────────

/**
 * Notify admin that a tutor group link was opened.
 */
async function _notifyAdminTutorGroupOpened(group, visitorPhone) {
  try {
    const adminPhone = String(
      process.env.ZQ_ADMIN_PHONE || process.env.ADMIN_WHATSAPP_PHONE || ""
    ).replace(/\D/g, "");
    ...
    await sendText(
      adminPhone,
      `🧑‍🏫 *Tutor group link opened*\n\n` +
      `Group: *${group.name}* (${group.slug})\n` +
      `Views: ${(group.viewCount || 0) + 1}\n` +
      `Visitor: ${displayPhone}\n` +
      `Time: ${timeStr}`
    );
    console.log(`[TUTOR GROUP ADMIN] group opened - visitor ${visitorPhone} slug "${group.slug}"`);
  } catch (_) {
    // Non-critical
  }
}
...
/**
 * Notify admin that a visitor tapped a specific tutor inside a tutor group list.
 */
async function _notifyAdminTutorTapped(tutor, visitorPhone) {
  try {
    const adminPhone = String(
      process.env.ZQ_ADMIN_PHONE || process.env.ADMIN_WHATSAPP_PHONE || ""
    ).replace(/\D/g, "");
    ...
    const displayPhone = _formatPhoneDisplay(visitorPhone);
    const loc = tutor.location?.city ? ` · ${tutor.location.city}` : "";

    await sendText(
      adminPhone,
      `🧑‍🏫 *Tutor profile tapped from group link*\n\n` +
      `Tutor: *${tutor.businessName}*${loc}\n` +
      `Visitor: ${displayPhone}\n` +
      `Time: ${timeStr}`
    );
    console.log(`[TUTOR GROUP ADMIN] tutor tapped: "${tutor.businessName}" by ${visitorPhone}`);
  } catch (_) {
    // Non-critical
  }
}
...
/**
 * Notify admin when someone taps "List as a Tutor" in a tutor group.
 */
async function _notifyAdminTutorGroupRegTap({ group, visitorPhone }) {
  try {
    const adminPhone = String(
      process.env.ZQ_ADMIN_PHONE || process.env.ADMIN_WHATSAPP_PHONE || ""
    ).replace(/\D/g, "");
    ...
    await sendText(
      adminPhone,
      `🧑‍🏫 *New tutor registration tap!*\n\n` +
      `Group: *${group.name}* (${group.slug})\n` +
      `Phone: ${displayPhone}\n` +
      `Time: ${timeStr}\n\n` +
      `_This visitor tapped "List as a Tutor" and was sent into the registration flow._`
    );
    console.log(`[TUTOR GROUP ADMIN] reg tap - visitor ${visitorPhone} on "${group.slug}"`);
  } catch (_) {
    // Non-critical
  }
}