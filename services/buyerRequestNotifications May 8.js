...
//
// Uses the dedicated supplier_link_opened UTILITY template.
// Submit to Meta as UTILITY (triggered by buyer action, not ZimQuote-initiated):
//
...
//
// Works outside the 24-hour session window - ideal for cold visits via shared links.
// Falls back to sendButtons if template not yet approved.
// ─────────────────────────────────────────────────────────────────────────────
export async function notifySellerSmartLinkOpened({
  ...
  // ── Try dedicated UTILITY template (works outside 24hr window) ──────────────
  try {
    await _sendTemplate(normalizedPhone, "supplier_link_opened", [
      businessName || "Your business",
      sourceLabel,
      timeStr
    ]);
    console.log(`[SMART LINK TPL] supplier_link_opened → ${normalizedPhone} (via ${sourceLabel})`);
    return;
  } catch (templateErr) {
    console.warn(`[SMART LINK TPL] template failed for ${normalizedPhone}: ${templateErr.message}. Falling back to sendButtons.`);
  }
  ...
  try {
    ...
    console.warn(`[SMART LINK TPL] all fallbacks failed for ${normalizedPhone}: ${fallbackErr.message}`);
  }
}
// ─────────────────────────────────────────────────────────────────────────────
